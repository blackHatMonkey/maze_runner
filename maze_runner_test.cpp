/*
 * To compile: g++ maze_runner.cpp maze_runner_test.cpp maze.cpp -std=c++0x
 *
...
bool checkPath(int path[], int pathLength, int correctPath[],
               int correctLength);
void printPath(const std::string &fname, int path[], int pathLength, int rows,
               int cols);
...
int main(void) {
  ...
  if (numPassed == numTests) {
    std::cout << "Congratulations! You have passed testing for A1 part 3"
              << std::endl
              << std::endl;
    std::cout << "The paths generated by your function have been written to "
                 "your folder"
              << std::endl;
    std::cout << "The files are named test<number>path.txt" << std::endl
              << std::endl;
    std::cout << "To see what they look like: " << std::endl;
    std::cout << "   * go to: "
                 "https://seneca-btp500f19.github.io/btp500-f19/test3.html"
              << std::endl;
    std::cout << "   * click the radio button to load the maze for the test"
              << std::endl;
    std::cout
        << "   * click the browse button to load the path file for that test"
        << std::endl;
  } else {
...
bool test1(std::string &error) {
  ...
  if (!checkPath(path, pathLength, correct, correctLength)) {
    ...
    if (pathLength != correctLength) {
      ...
      error += "https://seneca-btp500f19.github.io/btp500-f19/test3.html\n";
    }
  }
...
bool test5(std::string &error) {
  ...
  int correct[45] = {199, 179, 178, 177, 197, 196, 195, 175, 174, 173, 172, 171,
                     151, 131, 111, 112, 92,  72,  52,  53,  33,  13,  12,  11,
                     10,  9,   29,  28,  27,  47,  67,  66,  86,  85,  84,  83,
...
bool test6(std::string &error) {
  ...
  int correct[213] = {
      1740, 1680, 1681, 1682, 1622, 1562, 1563, 1503, 1504, 1444, 1443, 1383,
      1382, 1381, 1321, 1322, 1323, 1324, 1264, 1204, 1205, 1145, 1085, 1084,
      1083, 1023, 1022, 1082, 1081, 1021, 961,  901,  841,  842,  782,  722,
      723,  724,  664,  604,  605,  606,  666,  667,  607,  547,  487,  488,
      489,  429,  430,  370,  371,  372,  312,  313,  253,  254,  255,  195,
      196,  197,  198,  199,  259,  319,  318,  317,  377,  378,  379,  439,
      499,  498,  497,  496,  556,  616,  676,  736,  796,  795,  855,  915,
      975,  1035, 1095, 1094, 1154, 1155, 1156, 1216, 1276, 1336, 1337, 1397,
      1398, 1458, 1459, 1460, 1520, 1580, 1581, 1521, 1522, 1582, 1583, 1584,
      1585, 1586, 1526, 1527, 1528, 1468, 1467, 1407, 1408, 1409, 1410, 1411,
      1351, 1352, 1412, 1413, 1414, 1354, 1294, 1293, 1292, 1291, 1231, 1232,
      1172, 1173, 1174, 1114, 1115, 1055, 1056, 996,  997,  998,  999,  939,
      879,  878,  877,  817,  816,  815,  814,  754,  694,  634,  633,  632,
      572,  512,  513,  514,  515,  516,  576,  636,  696,  756,  757,  697,
      698,  638,  639,  579,  519,  520,  521,  522,  523,  583,  584,  585,
      645,  646,  647,  648,  649,  650,  651,  591,  590,  530,  529,  469,
      409,  349,  350,  290,  230,  231,  232,  172,  112,  113,  173,  233,
      234,  235,  236,  176,  116,  56,   57,   58,   59};
...
bool test8(std::string &error) {
  ...
  int correct[174] = {
      423,  424,  484,  483,  543,  603,  604,  605,  606,  666,  667,  607,
      547,  487,  488,  489,  429,  430,  370,  371,  372,  312,  313,  253,
      254,  255,  195,  196,  197,  198,  199,  259,  319,  318,  317,  377,
      378,  379,  439,  499,  498,  497,  496,  556,  616,  676,  736,  796,
      795,  855,  915,  975,  1035, 1095, 1094, 1154, 1155, 1156, 1216, 1276,
      1336, 1337, 1397, 1398, 1458, 1459, 1460, 1520, 1580, 1581, 1521, 1522,
      1582, 1583, 1584, 1585, 1586, 1526, 1527, 1528, 1468, 1467, 1407, 1408,
      1409, 1410, 1411, 1351, 1352, 1412, 1413, 1414, 1354, 1294, 1293, 1292,
      1291, 1231, 1232, 1172, 1173, 1174, 1114, 1115, 1055, 1056, 996,  997,
      998,  999,  939,  879,  878,  877,  817,  816,  815,  814,  754,  694,
      634,  633,  632,  572,  512,  513,  514,  515,  516,  576,  636,  696,
      756,  757,  697,  698,  638,  639,  579,  519,  520,  521,  522,  523,
      583,  584,  585,  645,  646,  647,  648,  708,  768,  828,  829,  889,
      949,  950,  1010, 1011, 951,  952,  953,  893,  894,  895,  955,  1015,
      1075, 1076, 1077, 1078, 1018, 1019};
  bool rc = true;
  if (!checkPath(path, pathLength, correct, correctLength)) {
    rc = false;
    if (pathLength != correctLength) {
