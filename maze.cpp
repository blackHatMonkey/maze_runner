...
bool Maze::isBeside(int cell1, int cell2) const {
  ...
  if ((cell1 >= 0 && cell1 < numCells_) && (cell2 >= 0 && cell2 < numCells_)) {
...
int Maze::getCell(int row, int col) const { return row * cols_ + col; }

// marks a cell as being visited
void Maze::mark(int cellNumber) { cells_[cellNumber] = true; }

// returns the row number of the cell with cellNumber
int Maze::getRow(int cellNumber) const { return cellNumber / cols_; }
// returns the row number of the cell with cellNumber
int Maze::getCol(int cellNumber) const { return cellNumber % cols_; }

// unmarks a cell as being visited
void Maze::unMark(int cellNumber) { cells_[cellNumber] = false; }

// returns true if the cell Number is Marked as visited.
bool Maze::isMarked(int cellNumber) { return cells_[cellNumber]; }
