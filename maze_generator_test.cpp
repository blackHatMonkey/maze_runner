/*
 * To compile: g++ disjointset.cpp maze_generator_test.cpp maze_generator.cpp
 * -std=c++0x
 */
...
void printMaze(const std::string &fname, int maxRow, int maxCol, Wall walls[],
               int numWalls);
...
int main(void) {
  ...
  if (numPassed == numTests) {
    ...
    std::cout << "Some mazes generated by your function have been written to "
                 "your folder"
              << std::endl;
  } else {
...
/* test1: ensure that the number of walls is correct.  The number of walls
   depends number of rows/cols and number of cells.  So the same number of cells
   with different number of rows/cols will have different number of walls in
   solution For example suppose your maze had 6 cells with  1 row, 6 cols, the
   number of walls in solution is 0 in the solution set.  But if it was 2 rows,
   3 cols, the number of walls is 2
*/
bool test1(std::string &error) {
  int rc = true;
  Wall walls[50000];
  int result;
  int mazeSizes[10][2] = {{1, 6},   {6, 1},   {2, 3},     {20, 20}, {1, 100},
                          {100, 1}, {50, 50}, {100, 100}, {30, 60}, {40, 30}};
...
public:
  ...
  void unionSet(int rep1, int rep2) { sets_[rep2] = rep1; }
  int numSets() const {
    int total = 0;
...
bool checkWalls(int maxRow, int maxCol, Wall walls[], int numWalls) {
  ...
  bool rc = true;
  getAdjacentCells(maxRow, maxCol, walls, numWalls, adjacent, numAdjacent,
                   numCells);
  for (int i = 0; i < numCells; i++) {
...
bool sameMaze(int **adjacent1, int *numAdjacent1, int **adjacent2,
              int *numAdjacent2, int numCells) {
  int rc = true;
...
void printMaze(const std::string &fname, int maxRow, int maxCol, Wall walls[],
  ...
  fs << "[" << walls[numWalls - 1].cell1() << ", "
     << walls[numWalls - 1].cell2() << "]]}" << std::endl;
  fs.close();
}
...
bool test3(std::string &error) {
  ...
  for (int i = 0; rc && i < 5; i++) {
    ...
    getAdjacentCells(row, col, walls1, numWalls1, adjacent1, numAdjacent1,
                     numCells);

    int numWalls2 = generateMaze(row, col, walls2);
    int **adjacent2 = new int *[numCells];
    int *numAdjacent2 = new int[numCells];
    getAdjacentCells(row, col, walls2, numWalls2, adjacent2, numAdjacent2,
                     numCells);

    if (sameMaze(adjacent1, numAdjacent1, adjacent2, numAdjacent2, numCells)) {
      rc = false;
      error = "Error 3: Your generateMaze() function appears to be generating "
              "the same maze each time.\nIt should generate a different maze "
              "with each call\n";
...
bool test4(std::string &error) {
  ...
  bool result;
  int mazeSizes[8][2] = {{10, 20}, {20, 10}, {50, 50}, {6, 1},
                         {1, 6},   {30, 60}, {40, 30}, {70, 100}};
  for (int i = 0; i < 8; i++) {
    int row = mazeSizes[i][0];
    int col = mazeSizes[i][1];
    int numWalls = generateMaze(row, col, walls);
